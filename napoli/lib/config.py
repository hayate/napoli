...
class Config(Singleton):
    def __init__(self, module):
        """
        module (str): name of a python module or path to the file containing configuration parameters
        """
        if getattr(self, '__init', False):
            # do initialization here
            self.__data = self._load(module)

    def _load(self, config):
        if config.endswith('.py'):
            config = '.'.join(config.rsplit('.py')[0].strip(os.path.sep).split(os.path.sep))
        segs = config.rsplit('.')
